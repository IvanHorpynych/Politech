...
var
  ...
  iR,iG,iB:Byte;
begin
  mouse.FindVertex(x,panel1.height-y,Scene,point); // Получаем выбранный полигон
  for i:=1 to numFound do // Для всех полигонов рядом найденных по Mouse.FindVertex
  begin
  ...
  label17.caption:=intTostr(xxx);// Координаты мышки в OpenGL
  label18.caption:=intTostr(yyy);
  label5.caption:=intTostr(x); // Координаты X и
  label6.caption:=intTostr(y); // Y в текущем окне
  label21.caption:=intTostr(x-xxx); // Разность между OpenGL координатами и координатами окна
  label22.caption:=intTostr(y-yyy);
  If numFound=0 then // Если ничего не найдено в mouse.FindVertex то ничего не показываем
  begin
    label17.caption:='  ';
    label18.caption:='  ';
    label21.caption:='  ';
  ...
  label11.caption:=intTostr(numfound); // Кол-во выделенных полигонов
end;

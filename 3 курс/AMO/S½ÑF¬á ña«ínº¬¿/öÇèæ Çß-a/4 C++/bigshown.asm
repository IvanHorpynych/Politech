...
NewLine db 10,13,0	;10 - новий рядок, 13 - перехід у начало строки, 0 - термінальній нуль
Space db 32,0		; 32 - пробіл, 0 - термінальній нуль
Symbol db '%c',0	; рядок для другування символу, заданого параметром (dl)

.code		; розділ коду програми

printf PROTO arg1:Ptr Byte, printlist: VARARG	; прототип функції виводу
; Увага! printf змінює значення регістрів edx, ecx та eax

;*****************************************
; п/п виведення на екран в hex-форматі
; даних із регістра esi:
; якщо di=28, то виводяться всі 4 байта 
; якщо di=20, то виводяться 3 молодші байта 
; якщо di=12, то виводяться 2 молодші байта 
...
bt0:
		mov     edx,esi
		mov		cl,bl			; для shr не можна використовувати bl
		shr     edx,cl
		and     dl,00001111b
		cmp     dl,10
		jl      bt1
...
@mas         equ         [ebp+8] ; адреса адреси 
; len - кількість байт масива, які необхідно вивести на екран
@len         equ         [ebp+12] ; адреса кількості
...
;-----------------------------------------------
; обчислення кількості пропусків в першому рядку
;-----------------------------------------------
		mov     ax,@len
		test    ax,00000011b
		pushf
...
; формування пропусків по відсутніх подвійних словах
		mov     ah,8
		sub     ah,al
		mov     al,ah
		xor     ah,ah
...
; формування пропусків по відсутніх байтах у подвійному слові
		mov     dh,4
		xchg    dh,dl  ;dh -  1  2  3
		sub     dl,dh  ;dl -  3  2  1
		shl     dl,1   ;dl -  6  4  2
...
l000:
       ...
       ; виведення початковіх пропусків в першому рядку (ecx не зберігається)
l001:
...
l002:
		...
		inc     cx
@3:
		mov	ebx,@mas			; записуємо у ebx адресу масиву
		lea	ebx,[ebx+ecx*4]-4	; записуємо у ebx адресу останнього (найстаршого) елементу масиву

; виведення масиву (з найстаршого елементу до наймолодшого)
l004:
		mov			esi, dword ptr [ebx]	; у esi зберігаємо 4 байти масиву, які будемо виводити
		sub			ebx,4		; переміщуємо ebx на молодші 4 байти
		call        show_bt		; виклик функції виводу
		mov         di,28
		dec         cx
		test        cx,7		; 7 = 0111b
		jne         l005

...
double Spline::MyFuction(double x)
{
	//return 7.5 * log10(x) * sin(x);
	return 1.2*tanh(x)*cos(10*x)*x;
}

double* Spline::findAi(double N)
{
...
double Spline::Fi(int i, double N)
{
	double h = (b - a) / N;
	double x = a;
	double f_i;
...
double* Spline::findDi(double* C, double N)
{
...
double* Spline::findBi(double* C, double* D, double N)
{
...
double Spline::Splin(double x, double* A, double* B, double* C, double* D, double N)
{
	double h = (b - a) / N;
	int i = 0;
...
void Spline::SplineInterpolation(double N)
{
   double* A;
	double* C;
	double* D;

...
void Seidel::outM(std::vector<std::vector<double>> matrixO)
{
...
void Seidel::solution()
{

	addNr();   // add 0 row in matrix xi;
	ForNormM();
	xi_arr.push_back(xi);
	int n=0;

	printf("N  x1       x2        x3        x4       d1       d2       d3     d4       max_d\n");
	double ex=0;
  do
	{
		std::cout<<n<<" ";
...
double Seidel::ret_Max_D()
{
	double max=abs(max_d[0]);
	for (int i = 1; i < max_d.size(); i++)
	{
		if (max<abs(max_d[i])) max=abs(max_d[i]);
	}
	return max;
}


void Seidel::ForNormM()
{
	for (int i = 0; i < matrix.size(); i++)
	{
...
void Seidel::_xi_c(int i)
{
	double sum=matrix[i][0];
		...
		max_d.push_back(abs(xi[i]-sum/matrix[i][i+1]));

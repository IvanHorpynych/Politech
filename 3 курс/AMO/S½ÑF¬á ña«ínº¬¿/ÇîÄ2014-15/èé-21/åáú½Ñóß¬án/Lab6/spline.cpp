...
double* findCi(double N){
	...
	for (int i = 1; i < (N - 1); ++i)
		for (int j = 0; j <= N; ++j){
			...
			if ( j == (i - 1) ) 
				matrix[i * ((int)N + 1) + j] = h;
			if (j == i) 
				matrix[i * ((int)N + 1) + j] = 4 * h;
			if ( j == (i + 1) ) 
				matrix[i * ((int)N + 1) + j] = h;
			if (j == N) 
				matrix[i * ((int)N + 1) + j] = 6 * ( Fi(i - 1, N) - 2 * Fi(i, N) + Fi(i + 1, N) ) / h;
	}
	...
	for (int p = 0; p < (N - 1); ++p){
		tmp1 = matrix[p * ((int)N + 1) + p];
		for (int j = 0; j <= N; ++j)
			matrix[p * ((int)N + 1) + j] = matrix[p * ((int)N + 1) + j] / tmp1;

		for (int i = (p + 1); i < N; ++i){
			tmp2 = matrix[i * ((int)N + 1) + p];
			for (int j = 0; j <= N; ++j)
				matrix[i * ((int)N + 1) + j] = matrix[i * ((int)N + 1) + j] - matrix[p * ((int)N + 1) + j] * tmp2;
		}
	}
...
double* findBi(double* C, double* D, double N){
	...
	for (int i = 1; i < N; ++i)
		B[i] = (h * C[i] / 2) - (h * h * D[i] / 6) + ( Fi(i, N) - Fi(i - 1, N) ) / h;
	return B;
}
...
void SplineInterpolation(double N)
{
	...
	int k = (b - a) * 300; //кількість точок, які обчислюються на заданому інтервалі

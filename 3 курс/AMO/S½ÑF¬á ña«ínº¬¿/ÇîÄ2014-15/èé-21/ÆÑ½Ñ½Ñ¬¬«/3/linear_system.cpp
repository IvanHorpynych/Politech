...
void GaussSeidel(double Matrix[n][n], double RightPart[n], double x[n]){
     double tmpMatrix[n][n]; //матриц€ коеф≥ц≥Їнт≥в
	 double RightP[n]; //стовпець в≥льних член≥в
	 double d,s;
     ...
     do{
        ...
        for (int i = 0; i < n; i++){
            ...
            d = abs(temp - x[i]);//дос€гненн€ завершенн€ ≥терац≥йного 
         }                       //
     } while (eps < d);          //процесу √ауса-«ейдел€ при дос€гненн≥ точност≥ eps
...
void printMatrix(double Matrix[n][n], double RightPart[n], double x[n]){
	for (int i=0; i<n;i++){
		for (int j = 0; j < n; j++){
			cout.precision(2);
			cout << Matrix[i][j]<< fixed << setprecision(2) << "\t";
		}
		cout << RightPart[i] << "\t";
		cout << "x" << i+1 << "=" << x[i]  << endl;
	}

...
void append2list(CNode **head, const CNode *pn){
	...
	{
		(*head)->prev->next = p;
		p->prev = (*head)->prev;   //замыкаем
	}
	(*head)->prev = p;  //замыкаем список
	p->next = (*head);
...
void del_node(CNode **head, int id){
	...
	if (isempty(*head) == 0){
		while (tmp = get_node(*head, id)){
			tmp->prev->next = tmp->next;   //убираем этот елементт и замыкаем список

...
double L(int n, double x)
{
	...
	while(i<n)
	{
		Tn1 = (1.0*(2*i+1)/(i+1))*x*Tn - (1.0*i/(i+1))*Tn_1;
		Tn_1 = Tn;
		Tn = Tn1;
		++i;
	}
...
double Integral_g_L_RC(double (*f)(double),double a,double b, int k, double eps)
{
...
{	
	return Integral_g_L_RC(f,a,b,k,eps)*(2*k+1)/2;
}

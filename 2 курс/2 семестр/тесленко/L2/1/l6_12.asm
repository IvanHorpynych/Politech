...
setmous PROC   ;обробка подій миші
;-------------------------------------------------------------------------------
        pusha                                   ; записываем (размещаем) в стеке регистры общего назанчния
                ...
                mov cx, 1000b            ; единицами в соответствующих разрядах регистра CX. (1000b нажатие правой кнопки -- бит3 установлен 1)
        push es
...
setmous ENDP
;-------------------------------------------------------------------------------
retmous PROC   ;повернутись до виконання прогр
;-------------------------------------------------------------------------------
push ax
        xor cx, cx
        mov ax, 0ch                             ;функция 0Ch позволяет программно задать реакцию на события
...
;-------------------------------------------------------------------------------
pusha
;; пошук області, в якій знаходиться курсор
	...
	jle skip	;пропускаємо наступну команду (column<=40)
...
skip:
	...
	push ax
	push bx
	mov ax,_DATA  ;щоб могти звернутись до змінних з data		
	mov ds,ax
					
	mov    al, clock[bx]  ;масив з квантами всіх задач
	cmp    al, 240
	jb    @addiction
	mov 	al, 255
	jmp      @clock_1

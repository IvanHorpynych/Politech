...
;-------------------------------------------------------------------------------
pusha
		mov ax,0	;обнуляємо ax, в ax буде зберігатись номер задачі

		shr cx,3	;отримуємо координати в знакомісцях по осі х
		cmp cx,40	;якщо потрапляємо на першу половину рядка
		mov second_col,0

		jle skip	;пропускаємо наступну команду
		
		inc ax		;збільшуємо номер на 1
		mov second_col,1
	skip:
		shr dx,3	;отримуємо координати в знакомісцях по осі y
		mov row_number,dx

		add ax,dx	;"множимо" ax на 2
		add ax,dx

		mov di,ax	;отриманий номер задачі пишем в di


		;mov di,screen_addr[0]
		;add al,48
		;mov ah,1010b
		;stosw
                
		xor cx,cx	;обнуляємо регістри
		xor si,si

		mov cx,10	;цикл для пошуку перерваної задачі
	hesitation_search:
		cmp init[si],hesitation
		je find		;знайшли перервану

		inc si
		loop hesitation_search

	find:			;знайдена перервана
		cmp si,di	;якщо співпадає з поточною
		je exit		;виходимо

		cmp row_number,5	;якщо клік поза областю задач
		jge exit		;виходимо

		mov init[di],absent	;інакше встановлюємо статус absent

		mov di,screen_addr[0]	;берем зміщення виводу першої задачі
		mov ax,160		;вираховуємо зміщення виводу вибраної задачі
		imul ax,row_number
		add di,ax

		cmp second_col,1	;перевіряємо чи це другий стовпчик
		jne skip2
		add di,80		;прибавляємо половину екрана
	skip2:
		add di,6		;зміщення до цифр

		mov al,48		;код нуля
		mov ah,1010b 		;атрибути символа

		mov cx,22		;всього 22 біта
	loop_zeroing:
		stosw			;обнуляємо
		loop loop_zeroing

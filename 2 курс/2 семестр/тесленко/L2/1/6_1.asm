...
max_prg		equ	16			; максимальна кількість "одночасно" виконуваних  задач
...
STCK	SEGMENT	WORD	STACK	'stack'	use16
		dw    	32000 	dup	(?)
top		label	word
   		dw	100	dup	(?)	; резерв для помилок анти переповнення стека
STCK	ENDS

DATA1	SEGMENT	WORD	PUBLIC	'DATA'	use16
...
screen_addr 	dw  	16 dup (0) 		; адреса (зміщення від початку відеосторінки) області введення на екран значень задачі
...
CODE1	SEGMENT	BYTE	PUBLIC	'CODE'	use16
...
setint8		PROC
	...
	jnz	zero_8 				
	MOV	AH,35H          		; отримати вектор переривання переривання від таймера (8)
	MOV  	AL,8     	    		 
	INT    	21H                 		; значення що повертається: es:bx - логічна адреса системної процедури обробки переривання від таймера
        ...
        mov    	dx,offset userint8		; формування в ds:dx логічної адреси процедури користувача для обробки переривань від таймера
        push   	ds				 
        push   	cs				 
        pop    	ds

        MOV  	AH,25H    			; встановити вектор переривання від таймера
        MOV  	AL,8         			
	...
	jmp	$+2              		; стандартний метод узгодження швидкісного процесора з більш повільним зовнішнім пристроєм. Припускаємо, що
                                           	; "безглузда" команда jmp очищує буфер попередньої вибірки команд і, тим самим, уповільнює роботу процесора. Тим часом
...
retint8		PROC
	...
	MOV	AH,25H        			; відновити початковий вектор переривання від таймера
	MOV	AL,8             		
...
setint9		PROC
	...
	or	al,al
	jnz	zero_9
	mov	ah,35h         			; отримати вектор переривання переривання від клавіатури (9)
	mov	al,9              		
	int	21h             		; значення що повертається: es:bx - вказівник на системну процедуру обробки переривання від клавіатури

	mov	cs:int9ptr,bx    		; зберегти в сегменті кодів вказівник на системну процедуру
	mov	cs:int9ptr+2,es 		

	mov	dx,offset userint9		; формування в ds:dx логічної адреси процедури користувача для обробки переривань від клавіатури
	push	ds
	push	cs				
	pop	ds				; ds:dx - вказівник на процедуру користувача оброб. переривання від клавіатури

	mov   	ah,25h              		; встановити вектор "перехоплення" переривання від клавіатури (9)
	mov   	al,9                   		
	int	21h                     	 
	pop	ds
...
;---------------
	...
	mov	ax,data1			; в перерваній програмі вміст сегментного регістра
...
esc_key		equ	01h            		; скан-код клавіші esc
	...
	mov	ah,al				; 7-ий розряд дорівнює 0 при натисканні клавіші, 1- при відтисканні
	and	al,7fh     			
...
;---------------
	...
	pop	es
	popa
	jmp	dword ptr cs:int9ptr 		; перехід на системну процедуру обробки переривань від клавіатури, яка
                                                ; виконає всі необхідні дії, включаючи повернення в перервану програму
...
;==============================================
;  Процедура відображення скан-коду на екрані
;==============================================
...
begin:
	mov 	ax,data1			; дані записуємо до сегменту ds
	mov 	ds,ax
	...
	int   	10h
       	...
       	mov  	cx,max_prg			; cx = кількості програм
       	xor   	esi,esi				; esi = 0
       	mov  	bx,4				; початкова установка bx
